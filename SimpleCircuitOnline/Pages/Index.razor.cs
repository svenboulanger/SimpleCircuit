        ...
        protected async Task UploadFile(UploadEventArgs args)
        {
            ...
            if (!string.IsNullOrEmpty(args.Script) && (args.Version == null || StringComparer.Ordinal.Compare(args.Version, _simpleCircuitVersion) < 0))
            {
                if (args.Version == null)
                    _logger.Warnings.Add("The script and style were generated by an older version. If the circuit looks wrong, consider pressing the reset button in the Style tab.");
                else
                    _logger.Warnings.Add("The script and style were generated by an older version ({args.Version}). If the circuit looks wrong, consider pressing the reset button in the Style tab.");
            }
