...
namespace SimpleCircuitOnline.Pages
{
    public partial class Index
    {
        ...
        private readonly List<string> _queuedErrors = new(), _queuedWarnings = new();
        ...
        protected async Task UploadFile(UploadEventArgs args)
        {
            _logger.Clear();
            if (args.Errors != null)
                _logger.Errors.AddRange(args.Errors);
            if (args.Warnings != null)
                _logger.Warnings.AddRange(args.Warnings);

            // Check the version of uploaded files
            if (args.Version == null || StringComparer.Ordinal.Compare(args.Version, _simpleCircuitVersion) < 0)
            {
                if (args.Version == null)
                    _logger.Warnings.Add("The script and style were generated by an older version. If the circuit does not look correct, consider resetting the style.");
                else
                    _logger.Warnings.Add("The script and style were generated by an older version ({args.Version}). If the circuit looks incorrect, consider resetting the style.");
            }
        ...
        private void UpdateNow()
        {
            ...
            // Actual loading
            Task.Run(async () => _svg = await ComputeXml(false, _settings.RenderBounds)).ContinueWith(task => {
                lock (_lock)
                {
                    _loading = 0;
                }
        ...
        private async Task<XmlDocument> ComputeXml(bool includeScript, bool includeBounds = false)
        {
            ...
            try
            {
                ...
                if (ckt.Count > 0 && _logger.Errors.Count == 0)
