...
namespace SimpleCircuit.Parser
{
    ...
    {
        private readonly KeySearch<Func<string, IComponent>> _search = new KeySearch<Func<string, IComponent>>();
        ...
        public void RegisterAssembly(Assembly assembly)
        {
            foreach (var t in assembly.GetTypes())
            {
                ...
                if (attributes != null && attributes.Length > 0)
                {
                    ...
                    var factory = Expression.Lambda<Func<string, IComponent>>(Expression.New(ctor, p), p).Compile();
