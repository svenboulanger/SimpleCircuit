            ...
            protected override void Draw(IGraphicsBuilder builder)
            {
                ...
                switch (Variants.Select("sdomain", "zdomain"))
                {
                    case 0:
                        ...
                        builder.Text(span, new(-bounds.Left - bounds.Width * 0.5, -bounds.Bottom - 1), Vector2.UX, TextOrientationType.Transformed);
                        builder.Line(new(-2, 0), new(2, 0), textStyle);
                        span = builder.TextFormatter.Format("s", textStyle);
                        bounds = span.Bounds.Bounds;
                        builder.Text(span, new(-bounds.Left - bounds.Width * 0.5, -bounds.Top + 1), Vector2.UX, TextOrientationType.Transformed);
                        break;
                    ...
                    case 1:
                        ...
                        bounds = span.Bounds.Bounds;
                        builder.Text(span, new(-bounds.Left - bounds.Width * 0.5, -bounds.Bottom - 2), Vector2.UX, TextOrientationType.Transformed);
                        builder.Line(new(-2, -1), new(2, -1), textStyle);
                        span = builder.TextFormatter.Format("z^{-1}", textStyle);
                        bounds = span.Bounds.Bounds;
                        builder.Text(span, new(-bounds.Left - bounds.Width * 0.5, -bounds.Top), Vector2.UX, TextOrientationType.Transformed);
                        break;
